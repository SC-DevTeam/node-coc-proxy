...
class ServerCrypto extends Crypto {
    ...
    decryptPacket(message) {
        if (message.messageType == EMsg.ClientHello || (message.messageType == EMsg.LoginFailed && !this.getSessionKey())) {
            message.decrypted = message.payload;
        } else if (message.messageType == EMsg.Login || message.messageType == EMsg.LoginFailed) {
